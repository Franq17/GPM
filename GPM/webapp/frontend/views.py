...
def search():
    ...
    if keywords:
        ...
        flash(_('Ingrese el/los keyword(s)'), 'error')
...
def change_password():
    ...
    if form.validate_on_submit():
        ...
        flash(_("Su contraseña ha sido cambiada, por favor vuelva a loguearse"),
              "success")
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            flash('Revise su email en las instrucciones de '
                  'Cómo acceder a tu cuenta', 'success')
            ...
            flash(_('Lo siento, usuario no encontrado con este email.'), 'error')
