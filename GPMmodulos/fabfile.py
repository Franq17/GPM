...
def init(project="myapp"):
    ...
    with cd(project_dir):
        ...
        local("perl -pi -e 's/\"webapp\"/\"%s\"/g' setup.py webapp/configs/config.py" % project)
        # and *.py
        local("perl -pi -e 's/^from webapp/from %s/g' `find -iname '*.py'`" % project)
        # change fbone/ folder
        local("mv webapp %s" % project)

        # Deploy with mod_wsgi
        # configure wsgi
        local("perl -pi -e 's/webapp/%s/g' app.wsgi MANIFEST.in" % project)
        # configure vhost
        ...
        activate_this = os.path.join(project_dir, "/home/adolfo/pruebas-flask/bin/activate_this.py")
